# Python implementation of the Golden Search algorithm for use in Streamlit
import numpy as np
...
# define golden functions
def golden_calculation(a,b,tol):
    ...
    while (converged==0):
        ...
        outstr+=('iteration {0:5d} of golden section, domain length = {1:10.5f}\n'.format(ncalls,L))
...
def create_plot():
    ...
    plt.scatter(xpts, fpts, label='Golden Search Points', color='red', marker='x')
    ...
    plt.ylabel('y')
    plt.xlabel('x')
    plt.title('Golden Search on '+st.session_state.expression)
    plt.legend()
    return plt
...
st.title("Golden Search Algorithm Program")
st.write("This application enables you to experiment with golden search solution of 1-D equations")
...
xmin,fmin = golden_calculation(minx,maxx,tol)
